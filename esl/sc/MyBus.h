/*
 * combine SimpleBusLT and SimpleBusAT from SystemC TLM examples.
 */
...
public:
  ...
  typedef tlm_utils::simple_target_socket_tagged<MyBus>    target_socket_type;
  typedef tlm_utils::simple_initiator_socket_tagged<MyBus> initiator_socket_type;
...
public:
    ...
    mResponsePEQ("responsePEQ")    
  {
    for (unsigned int i = 0; i < NR_OF_INITIATORS; ++i) {
      target_socket[i].register_nb_transport_fw(this, &MyBus::initiatorNBTransport, i);      
      target_socket[i].register_b_transport(this, &MyBus::initiatorBTransport, i);
      target_socket[i].register_transport_dbg(this, &MyBus::transportDebug, i);
      target_socket[i].register_get_direct_mem_ptr(this, &MyBus::getDMIPointer, i);
    }
    for (unsigned int i = 0; i < NR_OF_TARGETS; ++i) {
      initiator_socket[i].register_nb_transport_bw(this, &MyBus::targetNBTransport, i);      
      initiator_socket[i].register_invalidate_direct_mem_ptr(this, &MyBus::invalidateDMIPointers, i);
    }
